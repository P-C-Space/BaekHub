...
## 풀이과정
* 1. N은 0~40 41개의 숫자 -> public static int N[] = new int[41]; // 0으로 초기화
* 2. 0.25초의 실행시간으로 진행된 연산을 기록해 두는 동적계획법으로 진행
* 3. 1은 N번째 피보나치의 수만큼 반복하는 것을 발견, 0은 N-1번째 피보나치 수만큼 반복
	* System.out.println(fibonacci(tmp-1)+" "+fibonacci(tmp));
* 4. N이 0일 때 0-1 인덱스 오류로 인해 오류처리
